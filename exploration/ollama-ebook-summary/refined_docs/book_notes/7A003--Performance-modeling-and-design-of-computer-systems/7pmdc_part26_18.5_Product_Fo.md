...
$$\text{Rate Leave} = \pi\left( c^{(1)}_1, c^{(2)}_1, ..., c^{(n_1-1)}_1 \right) \cdot \lambda_1 / \left( c(n_1)^1 \right) + \sum_{c} \lambda_1(c) \cdot \mu_1.$$x??
...
$$\pi(z_1, z_2, ..., z_k) = \frac{1}{k} \prod_{i=1}^k P(\text{state at server } i \text{ is } z_i),$$where $ z_i = \left( c^{(1)}_i, c^{(2)}_i, ..., c^{(n_i)_i} \right)$ and each server behaves like an M/M/1 classed queue.
x??
...
$$B_i = \pi(z_1, z_2, ..., z_k) \cdot \lambda_i / (c(n_1)^1) + \sum_{c} \lambda_i(c) \cdot \mu_i.$$x??
...
??x$$P\{\text{state at server } i \text{ is } z_i\} = (1 - \rho_i) \cdot \lambda_i / \left( c^{(1)}_i \right) \cdots \lambda_i / \left( c^{(n_i)_i} \right) \mu_{n_i}^i \times \prod_{j \neq i}^\infty \sum_{n_j=0} (1 - \rho_j) \cdot \lambda_{n_j}^j \mu_{n_j}^j.$$

Simplifying, this gives:
$$

P\{\text{state at server } i \text{ is } z_i\} = (1 - \rho_i) \cdot \lambda_i / \left( c^{(1)}_i \right) \cdots \lambda_i / \left( c^{(n_i)_i} \right) \mu_{n_i}^i.$$x??
...
Relevant equations:
- $E\left[ TC \right] = 0.3E[T|leaves after visiting 1 ] + 0.65E[T|loops back to 1 ] + 0.05E[T|loops back to 1 via 2 ]$- The equation expands into a recursive form that involves $ E\left[ TC \right]$ itself.
...
Relevant equations:
- $E\left[ TC \right] = E\left[ VC_{1} \right] \cdot E[T_1] + E\left[ VC_{2} \right] \cdot E[T_2]$ Where $E\left[ VC_i \right]$ are the expected virtual times at device 1 and 2.
...
- $E\left[ VC_1 \right] = 3 $-$ E\left[ VC_2 \right] = 1$ Then, use the equation for expected time in system:
...
Relevant equations:
- $E\left[ N_{C1} \right] = \frac{\rho_1}{1 - \rho_1} \cdot \frac{\lambda_{C1}}{\lambda_{C1} + \lambda_{I1}}$ Where $\rho_1 = \frac{\lambda_1}{\mu_1}$ and $\lambda_1 = \lambda_{C1} + \lambda_{I1}$.
...
Relevant equations:
- $E\left[ N_{C1} \right] = E\left[ \text{Number of jobs at CPU} \right] \cdot p $ Where$p$ is the fraction of those jobs that are CPU-bound jobs.

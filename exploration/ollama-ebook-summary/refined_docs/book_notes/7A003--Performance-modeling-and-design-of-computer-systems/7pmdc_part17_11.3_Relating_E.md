...
$$P(X > 5) = e^{-\frac{1}{10} \cdot 5} = e^{-\frac{1}{2}}$$??x
...
P(X > s + t | X > s) = P(X > t)$$:p Explain why the Exponential distribution's memory allows for easy calculation of conditional probabilities.
??x
...
P(A \text{ is the last}) = \frac{1}{2}$$??x
...
For $a = 20$:
$$E[X | X > 20] = 20 + E[X] = 20 + \frac{1}{\lambda}$$??x
...
$$Y \sim Geo(p = \lambda\delta)$$:p What is the expected value of /tilde{Y}, and how does this relate to the exponential distribution?
??x
...
??x
Given:
- Power supply lifetime $\sim \text{Exp}(1/500)$- Disk lifetime $\sim \text{Exp}(1/1000)$
...
$$N(t+s) - N(s)$$has the same distribution for all s. This is derived from the independent increments property, which ensures that the number of events in any interval [s, s+t] depends only on t.

...
Formula: 
$$D_i = \frac{V_i}{\sum_{j=1}^{S(i)} S(j)_i}$$:p How would you determine $ E[D_i]$ in practice?
??x
...
Formula:
$$\rho_i = X \cdot E[Si] = X \cdot E[V_i] \cdot E[S_i] = X \cdot E[D_i]$$:p
...
$$\rho_i = X \cdot Bi / C$$where $ X$ is the arrival rate of jobs per second.

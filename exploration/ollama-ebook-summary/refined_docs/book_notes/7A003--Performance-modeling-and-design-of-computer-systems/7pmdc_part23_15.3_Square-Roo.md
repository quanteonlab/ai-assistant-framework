...
- $R = \frac{\lambda}{\mu}$-$ c $is a constant that depends on the desired$ P_Q < \alpha $The value of$ c$ can be determined by solving the equation:
...
$$k^*_{\alpha} \approx R + c \sqrt{R}$$where $ R = \frac{\lambda}{\mu}$, and $ c$is a constant that depends on the desired threshold $\alpha$.
...
Where:
- $c$ is a constant
- $R = \frac{\lambda}{\mu}$-$\Phi(\cdot)$ is the CDF of the standard Normal distribution
...
Where:
- $\rho = \frac{\lambda}{k \mu}$-$ k = R + c \sqrt{R}$:p What is the significance of the constant $ c$ in determining the number of servers?
??x
...
- $\alpha = 0.8 \rightarrow c \approx 0.173 $-$\alpha = 0.5 \rightarrow c \approx 0.506 $-$\alpha = 0.2 \rightarrow c \approx 1.06 $-$\alpha = 0.1 \rightarrow c \approx 1.42$:p How does the square-root stafﬁng rule perform in practice, even for small server farms?
??x
...
$$E[T_Q] = \frac{1}{\lambda} \cdot P_Q \cdot \frac{\rho}{1 - \rho}$$:p How is the mean response time $ E[T_Q]$related to $ P_Q$?
??x
...
Where:
- $\rho = \frac{\lambda}{k \mu}$-$ k = R + c \sqrt{R}$---
#### Example Calculation
Given the ratio $R = 50 $, and a desired $ P_Q < 0.2$:
$$k^*_{\alpha} \approx 50 + 1 \cdot \sqrt{50} = 57$$:p What is an example calculation for determining the number of servers needed?
??x
...
- $P_{\text{block}} = P(XR=k)$-$ P(XR \leq k) \approx \frac{\phi(c)}{\sqrt{R} \Phi(c)}$ (15.6)
...
$$E[T] = \sum_{n=1}^{t_{\text{high}}} \frac{1.5}{1 - 1.5 n} + \frac{1.5 t_{\text{high}}^2}{2}$$x??
...
Relevant formula: 
$$X = \rho_{cpu} \cdot \mu_{cpu}$$:p What is the throughput,$ X$, in jobs per second?
??x
...
$$X = \rho_{cpu} \cdot \mu_{cpu} = 0.6 \cdot 4 \text{ jobs/sec} = 2.4 \text{ jobs/sec}$$```java
...
Relevant formula:
$$E[Tcpu] = \frac{1}{Xcpu}$$:p What is the expected time spent in the CPU,$ E[Tcpu]$?
??x
...
$$E[Tcpu] = \frac{1}{X_{cpu}} = \frac{1}{2.4} = 0.4167 \text{ seconds} \approx 0.41 \text{ seconds}$$x??
...
#### Relationship Between $\pi_j $ and$\pi_{*j}$ Background context:$\pi_j $ represents the steady-state probabilities of being in state$j $, while$\pi_{*j}$ is the same for the reverse process.
...
$$\pi_{*i} q_{*ij} = \pi_j \nu_j P_{ji} / (\pi_i \nu_i)$$where $ P_{ji}$is the probability of transitioning from state $ j$to state $ i$.

...
$$\sum_{j} P_{ij} = 1$$:p What does each row of the transition probability matrix represent?
??x
...
0.95 & 0.05 \\
0.40 & 0.60
\end{bmatrix}$$:p What are the states in the repair facility problem?
??x
...
1-p & p \\
p(1-p) & 1 - p
\end{bmatrix}$$:p How many states does the umbrella problem have?
??x
...
$$\lim_{n \to \infty} P^n_{ij} = \pi_j$$:p What does the limit of $ P^n_{ij}$as $ n$ approaches infinity represent?
??x
...
$$\sum_{i=0}^{M-1} \pi_i P_{ij} = \pi_j$$for all $ j $ and$$\sum_{i=0}^{M-1} \pi_i = 1.$$x??
...
$$\boldsymbol{\pi} P = \boldsymbol{\pi}$$and$$\sum_{i=0}^{M-1} \pi_i = 1.$$

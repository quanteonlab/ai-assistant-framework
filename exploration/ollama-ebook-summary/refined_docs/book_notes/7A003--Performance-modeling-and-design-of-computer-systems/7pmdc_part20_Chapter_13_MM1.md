...
$$\pi_0 = 1 - \frac{\lambda}{\mu} = 1 - \rho$$where ρ = λ/μ is the server utilization.
...
E[N] = \rho(1 - \rho) \cdot \frac{d}{d\rho} \left( \frac{1}{1 - \rho} \right) = \rho(1 - \rho) \cdot \frac{1}{(1 - \rho)^2} = \frac{\rho}{1 - \rho}$$x??
...
$$E[N] = \frac{\rho}{1-\rho} + \frac{p \cdot \rho_1 (1-\rho_1)}{(1-p) (1-\rho)}$$where $\rho = \lambda / \mu $ and$\rho_1 = \frac{\lambda p}{\mu}$.
...
$$E[N_Q] = \frac{\rho^2}{1-\rho}$$:p Prove the formula for the expected number of jobs $ E[N_Q]$ in an M/M/1 queue.
??x
...
$$\pi_n = (1 - \rho) \rho^n$$where $\rho = \frac{\lambda}{\mu}$ and $0 < \rho < 1$. For a finite capacity $ N$, the normalization condition ensures that the sum of all probabilities equals one. However, since the system can only have up to $ N$jobs:
...
$$\pi_n = (1 - \rho) \frac{\rho^n (1 - \rho^{N+1})}{1 - \rho} = \rho^n (1 - \rho^N)$$??x
...
??x
The loss probability $\psi $ can be found using the concept of PASTA (Poisson Arrivals See Time Averages). For a system where arrivals are Poisson, the fraction of jobs lost is equal to the steady-state probability that there are$N+1 $ or more jobs in the system.$$\psi = 1 - \sum_{n=0}^{N} \pi_n = (1 - \rho^N)$$
...
$$\text{Turnaway Rate} = \lambda (1 - \rho^N)$$??x
...
E[\text{Number in system}] = \frac{\lambda - \rho \mu}{\mu} + N(1 - \rho^N) - 1$$??x
...
#### Loss Probability with Different $\rho $:p Repeat the previous question but now $ N=5 $, and$\rho=\lambda/\mu = 0.8$.
??x
...
$$P\{N = n\} = \rho^n (1 - \rho), \quad n = 0, 1, 2, \ldots$$where $\rho = \frac{\lambda}{\mu}$ is the traffic intensity.
...
$$P\{N'/n\} = \rho^{n-1} (1 - \rho)$$:p The distribution N/prime has a name. What is the name of the distribution and what is its parameter?
??x
...
$$T = S_1 + S_2 + \ldots + S_N + S_x$$where $ S_i $ is the service time of job $ i$, and all service times are exponentially distributed with mean 1/μ.
...
$$E[T] = \frac{1}{\mu - \lambda}$$and the variance of the response time can also be derived using properties of the exponential distribution.
...
$$P\{T > t\} = e^{-(\mu - \lambda)t}$$:p Derive the variance of response time.
??x

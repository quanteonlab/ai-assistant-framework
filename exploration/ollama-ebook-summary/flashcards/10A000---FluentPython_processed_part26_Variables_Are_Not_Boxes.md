# Flashcards: 10A000---FluentPython_processed (Part 26)
...
Background context: In Python, a variable is simply a label pointing to an object. An object can have multiple labels (variables) assigned to it, which creates aliases. The `is` operator checks if two variables refer to the same object, while the `id()` function returns the unique identifier of an object in memory.
:p What does the `is` operator and the `id()` function reveal about `lewis` and `charles`?
??x
The `is` operator confirms that `lewis` and `charles` refer to the same object, as both variables have the same ID. This means they are aliases of each other.
```python
...
Background context: While `is` checks if two variables refer to the same object, the `==` operator checks for equality of values. Two objects can have the same value but different identities.
:p How do you demonstrate that `alex` and `charles` are equal in terms of their contents but not identical?
??x
You can use both `==` to check if the dictionaries have the same content, and `is not` to confirm they are not the same object.
```python
charles = {'name': 'Charles L. Dodgson ', 'born': 1832}
...
Background context: In Python, the `id()` function returns a unique identifier for an object. This ID remains constant throughout the life of the object.
:p What does the `id()` function reveal about `charles` and `lewis` in Example 6-3?
??x
The `id()` function shows that both `charles` and `lewis` point to the same memory address, confirming they are aliases.
```python
charles = {'name': 'Charles L. Dodgson ', 'born': 1832}
lewis = charles
print(id(charles), id(lewis))     # Both IDs are identical
```
x??

---
#### Difference Between `is` and `==`
Background context: The `is` operator checks for identity (whether two variables refer to the same object), while `==` checks for equality of values.
:p How do you differentiate between an alias (`lewis`) and a separate object (`alex`) when they contain the same data?
??x
You use `is not` to check if the variables refer to different objects, even though their contents are the same.
```python
charles = {'name': 'Charles L. Dodgson ', 'born': 1832}
lewis = charles
alex = {'name': 'Charles L. Dodgson ', 'born': 1832, 'balance': 950}
print(alex == charles)    # True (values are equal)
print(alex is not charles)     # True (not the same object)
```
x??

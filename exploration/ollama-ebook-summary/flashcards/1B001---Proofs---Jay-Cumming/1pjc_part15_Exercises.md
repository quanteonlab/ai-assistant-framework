...
Given:
- Function $f(m; n) = (5m - 3n, 2n)$- Function $ g(m; n) = (3m + 2n, 4n - m)$:p What is the formula for $ f \circ g$?
...
- The first component of $g(m, n)$ is $3m + 2n$- The second component of $ g(m, n)$is $4n - m$ Using these components in $f$:
...
- For all $a \in A $, $ g(f(a)) = a $- For all$ b \in B $,$ f(g(b)) = b $The function$ g $is called the **inverse** of$ f $, denoted as$ f^{-1}$.
...
- Even numbers:$\{\ldots, -4, -2, 0, 2, 4, \ldots\}$- Odd numbers:$\{\ldots, -3, -1, 1, 3, 5, \ldots\}$ These sets are disjoint and their union covers all integers.
x??

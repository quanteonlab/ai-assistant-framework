...
- Left side: $1 $- Right side:$\frac{1(1+1)}{2} = \frac{2}{2} = 1 $ Since both sides are equal, the statement is true for$n = 1$.
x??
...
$$1 + 2 + 3 + ... + k = \frac{k(k+1)}{2}$$we assume the formula holds for some $ k$:
$$1 + 2 + 3 + ... + k = \frac{k(k+1)}{2}$$

Then we need to show that:
$$1 + 2 + 3 + ... + (k+1) = \frac{(k+1)(k+2)}{2}$$is true. We can do this by adding $ k+1$ to both sides of the inductive hypothesis:
...
In this example:
- Base case: Proved for $n = 1 $- Inductive step: If assumed to be true for$ k $, proved to be true for$ k+1 $Therefore, by induction, we conclude that for any$ n \in \mathbb{N}$:
$$1 + 2 + 3 + ... + n = \frac{n(n+1)}{2}$$x??

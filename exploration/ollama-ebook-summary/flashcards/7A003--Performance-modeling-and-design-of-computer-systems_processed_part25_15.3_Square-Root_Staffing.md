...
#### Hair Salon Queueing System
Background context: The hair salon example is used to illustrate a small CTMC with finite buffer space. It involves two routers in tandem, each with a limited capacity (in this case, 1 customer). Packets continue to occupy a buffer if the next buffer in the chain of routers is full.
:p What are the two questions we need to answer regarding the hair salon queueing system?
??x
The proportion of potential customers who enter the salon and the mean response time for an entering customer.

To answer these, we first need to set up the state space accurately. The state space should represent the number of customers in each chair without ambiguity.
??x
The state space is defined as follows: (0,0), (0,1), (1,0). However, this does not account for a customer finishing service and waiting in the next chair. Therefore, we need to include states like (b,1) where "b" stands for blocked.
??x
To properly model the system, we introduce state (b,1) representing a situation where one customer has finished service at chair 1 but cannot move on because chair 2 is occupied. The transition rates are as follows:
- From (0,1): Rate \(\mu_2\) to (1,0)
- From (1,0): Rate \(\lambda\) to (0,1) and rate \(\mu_1\) to (1,1)
- From (1,1): Rate \(\mu_2\) to (b,1) and rate \(\mu_1\) to (0,1)
??x
The balance equations for each state are:
- For state (0,0): \(\pi_{0,0} \cdot \lambda = \pi_{0,1} \cdot \mu_2\)
- For state (1,0): \(\pi_{1,0} \cdot \mu_1 = \pi_{0,0} \cdot \lambda + \pi_{1,1} \cdot \mu_2\)
- For state (0,1): \(\pi_{0,1} \cdot (\mu_2 + \lambda) = \pi_{1,0} \cdot \mu_1 + \pi_{b,1} \cdot \mu_2\)
- For state (1,1): \(\pi_{1,1} \cdot (\mu_2 + \mu_1) = \pi_{0,1} \cdot \lambda\)
- For state (b,1): \(\pi_{b,1} \cdot \mu_2 = \pi_{1,1} \cdot \mu_1\)

Additionally, the sum of all probabilities must equal 1: \(\sum \pi_{i,j} = 1\).
??x
Using these equations, we can solve for the steady-state probabilities \(\pi_{i,j}\). Once solved, we use PASTA to find the proportion of potential customers who enter the salon and Little's Law to determine the mean response time.

:p What is the proportion of potential customers entering the salon?
??x
The proportion of potential customers entering the salon is given by \(\pi_{0,0} + \pi_{0,1}\).

Explanation: By PASTA (Poisson Arrivals See Time Averages), the probability that an arrival sees 0 customers in the first chair is equal to the proportion of time there are 0 customers in the first chair.
??x
:p What is the mean response time for an entering customer?
??x
The mean response time \(E[T]\) for an entering customer can be calculated as:
\[ E[T] = \frac{E[N]}{\lambda_{arrival}} \]
where \(E[N]\) is the expected number of customers in the system and \(\lambda_{arrival}\) is the arrival rate that actually makes it through the network.

Explanation: Using Little's Law, we can calculate the mean response time by dividing the expected number of customers in the system by the effective arrival rate.
??x
---

#### Batch System with M/M/2 I/O
Background context: This example considers a system with one CPU queue and one I/O queue served by two disks. The goal is to determine the exact throughput for this system, specifically focusing on an M/M/2 model for the I/O queue.

:p Show the CTMC for this batch system.
??x
The CTMC state space includes states representing different numbers of jobs in both queues. For simplicity, we consider the following states:
- (0,3): No CPU jobs and 3 I/O jobs on Disk B
- (1,2): One CPU job and two I/O jobs between Disks A and B
- (2,1): Two CPU jobs and one I/O job on Disk A
- (3,0): Three CPU jobs with no I/O jobs

The transition rates are as follows:
- From state (i,j) to (i+1,j-1): Rate \(\mu_j\) for job completion from the disk serving \(j\) jobs
- From state (i,j) to (i-1,j+2): Rate \(\lambda_i\) for new CPU jobs arriving and I/O jobs starting on the other disk

The CTMC diagram should reflect these transitions.
??x
???
```java
public class BatchSystemCTMC {
    private double lambda; // Arrival rate of CPU jobs
    private double muA, muB; // Service rates of Disks A and B
    
    public void updateState(double[] currentState) {
        int i = currentState[0]; // Number of CPU jobs
        int j = currentState[1]; // Number of I/O jobs on Disk B
        
        if (i > 0 && j < 2) { // Move from (i, j) to (i-1, j+2)
            // Transition based on arrival rate lambda and service rates muA, muB
        }
        
        if (j > 0) { // Move from (i, j) to (i+1, j-1)
            // Transition based on service rates muA, muB
        }
    }
}
```
This pseudocode outlines the logic for updating the state of the CTMC in a batch system with M/M/2 I/O.
??x

# Flashcards: 10A005---C-Software-Design---Klaus-Iglberger_processed (Part 4)
...
Background context explaining the concept. When testing a function, you might be tempted to test its internal workings directly by calling private member functions from outside the class. However, this approach can lead to fragile tests that break with minor changes in the code.

:p What is white box testing and why should it generally be avoided?
??x
White box testing refers to a testing methodology where the tester has knowledge of the internal structure or workings of the software. In your example, directly testing `updateCollection()` via `addBlob()` could introduce dependencies on implementation details that might change over time, making such tests brittle and potentially unstable.

To avoid this, it's better to perform black box testing, which focuses solely on the inputs and outputs without referencing internal structures or workings of the software. This approach ensures that changes in internal logic do not break your tests as long as the expected behavior remains consistent.
x??

---
#### Black Box Testing

Background context explaining the concept. In contrast to white box testing, black box testing treats the system as a "black box" where you know only what inputs are given and what outputs should be produced without knowing the internal structure or workings.

:p What is black box testing and why is it preferred over white box testing?
??x
Black box testing involves designing tests based on the external specifications of a component or system. The tester does not need to understand the internal logic or implementation details; they only verify that the input produces the expected output. This approach is more robust because changes in the internal workings (like refactoring) do not affect the test as long as the behavior remains consistent.

Example:
```cpp
// Widget class with black box testing
class Widget {
public:
...
public:
    void testAddBlob() {
        Widget widget;
        Blob blob = {/* initialize with some values */};
        widget.addBlob(blob); // This should update the collection according to specification
        // Verify that the collection has been updated as expected
    }
};
```
x??
...
Background context explaining the concept. To ensure robust tests, you might need access to private member functions from outside the class. One way is to use a friend function or method.

:p How can we make a test a friend of the Widget class?
??x
Making a test a friend of the `Widget` class grants it full access to private members, allowing you to call private methods directly for testing purposes. This approach keeps your tests robust against changes in internal implementation details while ensuring they cover the expected behavior.

Example:
```cpp
// Making TestWidget a friend of Widget
class Widget {
public:
    // ...
private:
...
public:
    void testAddBlob() {
        Widget widget;
        Blob blob = {/* initialize with some values */};
        widget.addBlob(blob); // This should trigger updateCollection()
        // Verify that the collection has been updated as expected
        widget.updateCollection(/* pass necessary arguments */);
        // Add checks to ensure the collection is in the correct state
    }
};
```
x??

...
$$P(\text{Cheese} | \text{Second half of week}) = \frac{P(\text{Cheese and Second half})}{P(\text{Second half})} = \frac{\frac{2}{7}}{\frac{4}{7}} = \frac{2}{4} = \frac{1}{2}$$x??
...
Formula: 
$$P\{F|E\} = \frac{P\{E|F\} \cdot P\{F\}}{\sum_{j=1}^{n} P\{E|F_j\} \cdot P\{F_j\}}$$:p What does the Extended Bayes Law state and how is it used?
??x
...
$$p_X(0) = 1 - p$$
$$p_X(1) = p$$:p What does the Bernoulli distribution represent?
??x
...
$$p_X(i) = P(X=i) = \binom{n}{i} p^i (1-p)^{n-i}$$where $ i = 0, 1, 2, ..., n$.
...
$$F_X(a) = P(X \leq a) = \sum_{x \leq a} p_X(x)$$and$$

F_X(a) = P(X > a) = \sum_{x > a} p_X(x) = 1 - F_X(a)$$:p What is the probability mass function (p.m.f.) of a discrete random variable?
??x
...
1 & \text{if } x > b
\end{cases}$$:p What is the probability density function (p.d.f.) of the uniform distribution?
??x
...
\end{cases}$$:p What is the probability density function (p.d.f.) of the Pareto distribution?
??x
...
$$\text{Avg} = \frac{1}{7}(7 + 7 + 5 + 5 + 5 + 0 + 2)$$:p What is the formula for calculating the expectation of a discrete random variable?
??x
...
$$E[X] = 0 \cdot (1 - p) + 1 \cdot p = p$$:p What is the expected value of a Bernoulli random variable?
??x
...
- Discrete case:$E[X^i] = \sum x^i \cdot P\{X=x\}$- Continuous case:$ E[X^i] = \int_{-\infty}^{\infty} x^i \cdot f_X(x) \, dx $ The expectation of a function $ g(X)$ can be calculated as:
$$E[g(X)] = \sum g(x) \cdot P\{X=x\} \text{ (for discrete)}$$
$$

E[g(X)] = \int_{-\infty}^{\infty} g(x) \cdot f_X(x) \, dx \text{ (for continuous)}$$:p What is the expectation of a function $ g(X)$?
??x
...
Var(X) = E[X^2] - (E[X])^2$$:p What is the definition of variance for a random variable?
??x

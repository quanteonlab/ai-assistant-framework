...
- **Type a**: Jobs that are in the system with original sizes ≤$x $- **Type b**: Jobs originally larger than$ x $but now reduced to size$ x$
...
$$E[\text{Wait}(x)] = E[\text{WSRPT}_x] \cdot \frac{1}{1 - \rho_x}$$where $\text{WSRPT}_x $ is the work in the system relevant to an arrival of size$x$.
x??

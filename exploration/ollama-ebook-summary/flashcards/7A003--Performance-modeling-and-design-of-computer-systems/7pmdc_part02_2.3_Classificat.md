...
??x
The equations for determining $\lambda_i $ involve balancing the flow in with the flow out at each server. Specifically,$$\lambda_i = r_i + \sum_j \lambda_j P_{ji}$$where $ P_{ji}$is the probability that a job from server $ j$goes to server $ i$.
x??
...
#### Throughput in Non-Probabilistic Networks of Queues
...
??x
In networks where routing is not probabilistic, the system throughput $X $ can be calculated directly using the total arrival rate. Specifically,$$X = \lambda$$

For specific servers like Disk1 and Disk2, their throughputs are:
- $X_{Disk1} = 3\lambda $-$ X_{Disk2} = 2\lambda$ x??
...
$$T = R + Z$$where $ R $ is the response time and $ Z$ is the think time.
x??
...
#### Throughput in Closed Interactive Systems - Partly-Open Systems
...
$$E[\text{Slowdown}] = \frac{\sum T(j)}{\sum S(j)}$$where $ T(j)$is the response time and $ S(j)$is the size of job $ j$.

For FCFS, if $E[T] = 29\frac{1}{12}$ jobs/sec with given service times:
$$E[\text{Slowdown}] = \frac{\sum T(j)}{\sum S(j)}$$where the sizes are distributed as: 1 with probability $\frac{3}{4}$, and 2 otherwise.
x??

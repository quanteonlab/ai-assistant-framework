...
E[T_Q] = (1 - \rho) + \frac{\rho}{\lambda} \cdot E[S]$$x??
...
E[T_Q^2] = \frac{\lambda E[S^3]}{3(1 - \rho)^2} + \left(\frac{\lambda E[S^2]}{2(1 - \rho)}\right)^2$$- For FCFS:
$$
...
- For FCFS: 
  - $E[T_{FCFS}] = 1.5 $-$ E[Slowdown_{FCFS}] = 1.5$

- For PS: 
  - $E[T_{PS}] = 2 $-$ E[Slowdown_{PS}] = 2$
...
$$P(\text{arrival in } [t, t+dt]) = \lambda dt + o(dt)$$where $ o(dt)$represents higher-order terms that are negligible for small $ dt$. The job sizes follow a distribution with cumulative density function (CDF)$ F(s)$.
...
$$P(\text{arrival in } [t, t+dt]) = \lambda dt + o(dt)$$where $\lambda $ is the rate parameter and$o(dt)$ represents higher-order terms that are negligible for small $ dt $. This implies that the inter-arrival times follow an exponential distribution with mean $\frac{1}{\lambda}$.
...
$$f_e(x) = F(x) \frac{E[S]}{1 - \rho}$$where $ F(x)$is the CDF of job sizes and $ E[S]$ is the expected job size.
...
$$E[T(x)] = \frac{x}{1 - \rho}$$where $\rho = \frac{\lambda}{\mu}$.
...
$$E[T(x)] = \frac{x}{1 - \rho}$$where $\rho $ is the utilization factor, defined as$\rho = \frac{\lambda}{\mu}$.
...
= \int_0^w f_{sys}(w|job \, has \, age \, x) \cdot f_e(x) \, dx,$$where $ f_{sys}(·)$is the pdf of job sizes in the system and $ f_e(·)$ is the arrival pdf. Further simplifying:
$$f_{sys}(w) = \int_0^w f(w|job \, has \, size \geq x) \cdot f_e(x) \, dx
= \int_0^w f(w) F(x) \cdot f_e(x) \, dx,$$where $ F(x)$ is the cumulative distribution function (CDF). By (30.1), we have:
$$f_{sys}(w) = f(w) \frac{w}{E[S]},$$where $ E[S]$ is the expected service time.
...
#### Implications for Understanding FCFS

Background context: The transform equation for waiting time (delay) in the M/G/1/FCFS queue from (26.14) is given by:
$$\tilde{T}_{Q, \text{FCFS}}(s) = \frac{1 - \rho}{1 - \rho/s} \tilde{S}(s).$$:p How does the M/G/1/PS queue relate to FCFS in terms of waiting time?
??x
The M/G/1/PS queue has a simpler and more beautiful form for mean response time, which is $x \frac{1}{1 - \rho}$. In contrast, the M/G/1/FCFS queue's waiting time transform is more complex and involves the service distribution $ S(s)$.
...
$$\tildewider{T}_{QFCFS}(s) = \sum_{k=0}^{\infty}(1-\rho)\rho^k\left(\frac{\tilde{S}_e(s)}{\rho}\right)^k.$$:p What does $\left(\frac{\tilde{S}_e(s)}{\rho}\right)^k$ represent?
??x $\left(\frac{\tilde{S}_e(s)}{\rho}\right)^k $ is the Laplace transform of the sum$\sum_{i=1}^k S(i)_e $, where $ S(i)_e, \; 1 \leq i \leq k $, are independent and identically distributed (i.i.d.) instances of$ S_e$. Kleinrock [110] notes that this formulation is peculiar within the context of M/G/1/FCFS queue analysis.
x??

#### Understanding $\tildewider{T}_{QFCFS}(s)$ We derive an expression for the mean delay in an FCFS system using the concept of work-in-system.
...
$$E[T(x)]_{PLCFS} = \frac{x}{1-\rho}$$and$$

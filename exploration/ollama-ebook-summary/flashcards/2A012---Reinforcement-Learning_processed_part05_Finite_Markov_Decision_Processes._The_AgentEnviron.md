...
#### Adding Constant Rewards to Gridworld
Background context: In Exercise 3.15, we consider a gridworld where rewards are positive for goals, negative for hitting an edge, and zero otherwise. We want to understand if adding a constant \( c \) to all these rewards affects the relative values of states under any policies.
Formula: The value function \( v_\pi(s) \) is defined by Equation (3.8), which relates state values to action values.

:p What happens when you add a constant \( c \) to all rewards in the gridworld?
??x
Adding a constant \( c \) to all rewards does not affect the relative values of any states under any policies. The value function changes by a constant, but the differences between different state values remain the same.

To prove this using Equation (3.8):
\[ v_\pi(s) = R(s) + \gamma \sum_{s'} P(s' | s, a)v_\pi(s') \]

If we add \( c \) to all rewards \( R(s) \), then:
\[ v_\pi(s) + c = (R(s) + c) + \gamma \sum_{s'} P(s' | s, a)(v_\pi(s') + c) \]
\[ = R(s) + \gamma \sum_{s'} P(s' | s, a)v_\pi(s') + c(1 + \gamma) \]

The term \( c(1 + \gamma) \) is a constant and does not affect the relative values of states. Therefore:
\[ v_\pi(s) = R(s) + \gamma \sum_{s'} P(s' | s, a)v_\pi(s') - c \]
Thus, adding \( c \) to all rewards shifts all value functions by the same amount.

The difference in values between any two states remains unchanged.
x??

---

#### Episodic Task and Constant Rewards
Background context: In Exercise 3.16, we consider an episodic task like maze running where a constant \( c \) is added to all rewards. We want to determine if this change affects the task.

:p Adding a constant \( c \) to all rewards in an episodic task such as maze running would have what effect?
??x
Adding a constant \( c \) to all rewards in an episodic tasks, such as maze running, does not leave the task unchanged. Unlike in continuing tasks where adding a constant affects only the value function by that same constant and leaves relative values unaffected, in episodic tasks, it changes the total reward collected per episode.

For example, if \( c \) is positive, episodes will take longer to complete as more steps are taken due to the added rewards encouraging exploration. Conversely, negative \( c \) would make episodes shorter. Thus, the nature and outcome of each episode change significantly.

Here's a simple pseudocode for updating total reward in an episodic task:
```java
int totalReward = 0;
while (!episodeComplete) {
    takeAction();
    observeReward(reward);
    totalReward += reward + c; // Add constant to observed reward
}
```
x??
...
#### Golf Example: State-Value Function for Putting
Background context: In Example 3.6, the state is the location of the ball on a golf course. The action value function \( q^*(s, \text{putter}) \) shows values based on using the putter to hit the ball.

:p What is the Bellman equation for action values in this golf example?
??x
The Bellman equation for action values (q-values) \( q^\pi(s, a) \) relates it to possible successor states and actions. In the context of Example 3.6:
\[ q^*(s, \text{putter}) = R(s, \text{putter}, s') + \gamma v^*(s') \]

Where:
- \( R(s, \text{putter}, s') \) is the immediate reward for moving from state \( s \) to successor state \( s' \).
- \( v^*(s') \) is the value of the next state under the optimal policy.

To elaborate further, if using the putter in state \( s \), we calculate:
\[ q^*(s, \text{putter}) = -1 + \gamma v^*(\text{next\_state}) \]

This equation recursively considers the immediate reward and the value of the next state under optimal policy.
x??

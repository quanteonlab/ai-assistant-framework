# Flashcards: Materials-Science-and-Engineering-An-Introduction_processed (Part 17)
...
#### Tube Volume Calculation

Background context: The volume of the cylindrical tube is calculated to find its mass and subsequently, cost. The formula involves the cross-sectional area of the annulus formed by the inner and outer radii of the tube.
...
:p What is the formula for calculating the volume of a cylindrical tube?
??x
The volume ($V$) of the cylindrical tube can be calculated using the annular area formula:
$$V = \pi (r_o^2 - r_i^2) L$$

Given $r_o = r_i + t$, we can substitute and simplify to get:
$$V = \pi [(r_i + t)^2 - r_i^2] L = \pi (2 r_i t + t^2) L$$x??

---

#### Mass Calculation for Each Alloy

Background context: Once the volume is known, it is multiplied by the density of each alloy to find its mass. The cost is then calculated by multiplying this mass by the unit mass cost.

Relevant formula:
$$

Cost = (V \cdot \rho) / 1000 \cdot c$$

Where:
- $V$: Volume in cubic meters
- $\rho$: Density of the alloy in g/cm³
- $c $: Unit mass cost in $ US/kg

:p How is the cost for each alloy calculated?
??x
The cost for each alloy can be calculated by first converting the volume from m³ to cm³, then multiplying by the density and unit mass cost. The formula used is:
$$Cost = (V \cdot \rho) / 1000 \cdot c$$

Where $V $ is in m³,$\rho $ in g/cm³, and$c $ in$ US/kg.

Example for steel:
- Volume ($V$) = 6.28 × 10⁻⁴ m³
- Density ($\rho$) of Steel = 7.8 g/cm³
- Unit mass cost (c) of Steel = 1.25 $US/kg

Cost calculation for steel:
$$Cost_{steel} = \left(6.28 \times 10^{-4} \cdot 7.8 \right)/1000 \cdot 1.25$$x??

---

#### Selection of the Least Expensive Alloy

Background context: After calculating the cost for each alloy, the least expensive one is chosen based on their unit mass costs.

:p Which alloy was determined to be the most cost-effective for the pressurized tube?
??x
Steel was determined to be the most cost-effective alloy for the pressurized tube with a calculated cost of$6.10 per kg.
...
Background context: Hardness is a measure of a material's resistance to localized plastic deformation. Common hardness testing techniques include Rockwell and Brinell tests.

:p What are the common hardness testing methods, and how do they differ?
??x
Common hardness testing methods include:

- **Rockwell Test**: Uses a diamond cone or steel ball indenter with a heavy initial load.
- **Brinell Test**: Uses a hardened steel ball indenter with a higher initial load.

These tests provide an indication of the material's resistance to localized plastic deformation, but they can differ in their scale and application:

```java
public class HardnessTests {
    public static double calculateRockwellHardness(double force, double diameter) {
        return (100 / Math.sqrt(Math.PI * 2.20462 * diameter)) - 10;
    }

    public static double calculateBrinellHardness(int loadInKg, double diameterOfDent) {
        return (3086.5 * loadInKg) / (Math.PI * Math.pow(diameterOfDent / 2, 2));
    }
}
```
x??
...
---
#### Specimen Cross-Sectional Area Parameters
Background context explaining the different cross-sectional area parameters used in material testing. These parameters are crucial for understanding the behavior of materials under load.

:p What does $A_0$ represent in material testing?
??x $A_0$ represents the specimen cross-sectional area prior to the application of a load. This value is important because it serves as a baseline reference point from which other measurements can be compared, ensuring consistency and accuracy in stress calculations.
...
public class MaterialTest {
    ...
    public void setInitialArea(double initialArea) {
        this.initialArea = initialArea; // Sets the initial cross-sectional area of the specimen before loading.
    }
}
```
x??

...
- $LD = \frac{\text{number of atoms centered on direction vector}}{\text{length of direction vector}}$- In an FCC unit cell, the [110] direction passes from the center of atom X through atom Y to the center of atom Z.
...
$$\sin \theta = \frac{n \lambda}{2 d_{hkl}}$$where $ n $is the order of reflection,$\lambda $ is the wavelength of the x-rays, and$d_{hkl}$ is the interplanar spacing.

# Flashcards: 10A008---Computational-Physics---Rubin-H_-Landau_processed (Part 48)
...
#### Delta-Shell Potential Definition and Momentum-Space Representation
...
#### Schrödinger vs. Lippmann–Schwinger Equations
...
$$R(k', k) = V(k', k) + \frac{2\pi}{i} \mathcal{P} \int_0^\infty dp \frac{p^2 V(k', p) R(p, k)}{(k_0^2 - p^2)/2m},$$is specifically designed to handle scattering problems. It includes a term for the potential $ V $ and an integral involving the reaction matrix $ R$, which describes how waves scatter off the target.
...
#### Principal Value Prescription for Singularities
...
#### Hilbert Transform and Its Application in Integrals
...
$$\mathcal{P}\int_{-\infty}^{+\infty} \frac{dk}{k-k_0} = 0.$$:p How does the principal value of a function like $1/(k-k_0)$ simplify in integration?
??x
...
#### Scattering in Momentum Space with Delta-Shell Potential

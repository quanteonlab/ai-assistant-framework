# Flashcards: 2B002---Fundamentals-of-Data-Engineering_processed (Part 14)
...
Background context: The distributed monolith pattern is a way of organizing services within a distributed system where different tasks are performed by separate services, but these services share a common codebase or dependencies. This approach aims to leverage the benefits of distributed systems while maintaining some level of shared architecture.
:p What is the key characteristic of the distributed monolith pattern?
??x
The key characteristic is that it runs a distributed system with different services performing distinct tasks, yet shares a common set of dependencies or codebase.
x??

---
#### Hadoop Cluster Example
Background context: An example provided in the text discusses how a traditional Hadoop cluster hosts multiple frameworks and components. The cluster has internal dependencies and core Hadoop components like Hadoop common libraries, HDFS, YARN, and Java. It operates under the standard that a typical on-premises Hadoop system manages a unified environment for all users and jobs.
:p What is an example of a distributed monolith pattern described in the text?
??x
An example of a distributed monolith pattern described is a traditional Hadoop cluster, which hosts several frameworks such as Hive, Pig, or Spark. The cluster also runs core Hadoop components like Hadoop common libraries, HDFS, YARN, and Java.
x??

---
#### Challenges in Managing a Distributed Monolith
Background context: Managing upgrades and installations in a distributed monolith can be complex due to the shared codebase and dependencies among services. This challenge is exacerbated when multiple jobs need to run with different versions of frameworks or APIs, potentially leading to conflicts.
:p What are some challenges associated with managing a distributed monolith?
??x
Some challenges include managing upgrades and installations, as all services share the same codebase and dependencies. Conflicts arise when forcing jobs to upgrade dependencies might break them, or maintaining multiple versions requires extra complexity.
x??

---
#### Ephemeral Infrastructure Solution
Background context: To address the limitations of a distributed monolith, one solution is ephemeral infrastructure in a cloud setting where each job gets its own temporary server or cluster installed with necessary dependencies. This approach reduces conflicts by isolating jobs into separate environments.
:p What is an example of a solution to managing a distributed monolith?
??x
An example of a solution is using ephemeral infrastructure in a cloud setting, such as Amazon EMR and Google Cloud Dataproc, where each job gets its own temporary server or cluster with the necessary dependencies installed. This approach isolates jobs into separate environments, reducing conflicts.
x??

---
#### Containerized Decomposition Solution
Background context: Another solution to the distributed monolith's challenges is properly decomposing the system into multiple software environments using containers. This method allows for better isolation and management of dependencies without requiring all services to share a common codebase or environment.
:p What is another solution mentioned in the text?
??x
Another solution is to properly decompose the distributed monolith into multiple software environments using containers, allowing better isolation and management of dependencies.
x??

---
#### Evaluation Criteria for Monoliths vs. Modular Options
Background context: The text suggests evaluating monolithic architectures against modular options considering factors such as ease of understanding, reduced complexity, but also potential loss of flexibility, opportunity costs, and high-friction development cycles.
:p What criteria should be considered when choosing between monoliths and modular options?
??x
Criteria to consider include the ease of understanding and reduced complexity of monolithic architectures versus their potential drawbacks like lost flexibility, opportunity costs, and high friction in development cycles.
x??

---

#### Serverless vs. Servers
Serverless computing is a cloud computing execution model where the cloud provider runs the infrastructure, including servers and operating systems, on behalf of the customer. This reduces the need for server management tasks such as capacity planning, software patching, and hardware maintenance. The primary benefits are cost efficiency and increased flexibility.

:p What are the main reasons for the popularity of serverless computing?
??x
Serverless computing gained popularity primarily due to its cost-effectiveness and convenience. Instead of paying for a server regardless of usage, developers only pay when their code is invoked. This can significantly reduce costs for applications that have sporadic or unpredictable workloads.

:p How does AWS Lambda fit into the serverless ecosystem?
??x
AWS Lambda is one of the key players in the serverless computing space introduced by Amazon Web Services (AWS) in 2014. It allows users to run code without provisioning or managing servers, focusing on writing and deploying small pieces of code that are triggered by events.

:p What are some use cases where serverless might not be a good fit?
??x
While serverless offers significant benefits, it may not always be the best solution for certain applications. For example, handling high event rates with one function per call can lead to inefficiencies and higher costs compared to more traditional approaches like multithreading or multiprocessing.

:p What is BigQuery in Google Cloud Platform (GCP)?
??x
BigQuery in GCP is a serverless data warehousing service that enables querying large datasets using standard SQL. Users do not need to manage any backend infrastructure, as the system automatically scales to handle large queries and processes. Users pay for the amount of data consumed by their queries and for storage.

:p What are containers and how do they differ from virtual machines (VMs)?
??x
Containers are a method of software packaging that includes application code and its dependencies into a standardized unit, making it easier to deliver and run applications consistently across different environments. Containers share the host operating system kernel but include their own isolated filesystem and processes. They provide lightweight virtualization without the overhead of an entire OS.

:p How does Kubernetes fit into the container ecosystem?
??x
Kubernetes is a popular open-source platform for managing containerized workloads and services, facilitating both deployment and scaling. It helps automate tasks such as replication, update management, and rolling updates. Kubernetes enables fine-grained resource allocation among multiple containers on a single hardware node, making efficient use of resources.

---

#### Kubernetes as a Serverless Environment
Kubernetes, despite being managed by teams, allows for microservice deployment without worrying about hardware details. It supports containerization, which is seen as a partial solution to distributed monoliths.

:p What does Kubernetes offer developers and operations teams regarding microservices?
??x
Kubernetes provides a platform where developers can deploy microservices without needing to manage the underlying infrastructure. This abstraction layer simplifies deployment processes by handling container orchestration tasks such as scaling, networking, and storage.
...
Containers offer a degree of isolation through namespaces, but they are not as secure or isolated as full virtual machines (VMs). Container escape vulnerabilities can pose significant security risks.

:p How do containers provide security compared to VMs?
??x
Containers use namespaces and cgroups for resource isolation at the operating system level. However, this is less secure than a full VM because if an exploit allows code in a container to gain higher privileges on the host OS, it could escape the container boundaries, leading to potential security breaches.

```java
public class ContainerEscapeExample {
    public void demonstrateContainerEscape() {
        // Code that might lead to privilege escalation
        String command = "sudo docker exec -it container_id bash";
...
#### Containerized Function Platforms
These platforms run containers as ephemeral units triggered by events, providing the simplicity of AWS Lambda with more flexibility than traditional serverless frameworks.

:p What is a key difference between containerized function platforms and conventional serverless services?
??x
Containerized function platforms offer more control over runtime environments compared to highly restrictive functions-as-a-service (FaaS) like AWS Lambda. Users can run full-fledged containers with all necessary dependencies, offering greater flexibility in deployment scenarios.

```java
public class EventDrivenFunction {
    public void handleEvent(String eventJson) {
        // Logic to process the event and execute tasks within a container
        try {
            String[] cmd = {"docker", "run", "--rm", "-e", "EVENT=" + eventJson, "my-function-image"};
            ProcessBuilder processBuilder = new ProcessBuilder(cmd);
            Process process = processBuilder.start();
            // Handle output or errors from the container
        } catch (IOException e) {
            throw new RuntimeException("Failed to start container for function.", e);
        }
    }
}
```
...
#### Cost Comparison of Serverless vs. Traditional Servers
The cost model of serverless can become less advantageous when usage and maintenance costs exceed those of a traditional server setup.

:p Under what circumstances might using servers be more economical than serverless?
??x
Using servers becomes more attractive when the usage pattern leads to high ongoing costs associated with scaling in serverless models. For instance, if the application has sporadic bursts of traffic that are not cost-effective with serverless pricing models, running a traditional server setup could be more economical.

```java
public class CostComparison {
    public boolean shouldUseServerInsteadOfServerless(double estimatedUsageCost, double serverCost) {
        return (estimatedUsageCost > serverCost);
    }
}
```
x??
...
#### Customization and Control Over Servers
Servers offer greater customization and control compared to serverless frameworks. However, this comes with the risk of introducing vulnerabilities if not managed properly.

:p Why might developers prefer using servers over serverless environments?
??x
Developers may prefer traditional servers for several reasons: full control over the environment, ability to customize the runtime setup, and better handling of complex use cases that are not well-suited for highly restricted functions-as-a-service. However, they must be cautious about managing these resources securely.

```java
public class ServerManagement {
    public void manageServerResources() {
        // Logic to handle server failover, resource scaling, etc.
        String command = "sudo docker ps -a";
        ProcessBuilder processBuilder = new ProcessBuilder(command.split(" "));
        try {
            Process process = processBuilder.start();
            BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
            ...
            throw new RuntimeException("Failed to list server resources.", e);
        }
    }
}
```

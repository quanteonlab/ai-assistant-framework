# Flashcards: 2B002---Fundamentals-of-Data-Engineering_processed (Part 1)
...
#### Data Warehousing Origins (1980-2000)
Background context: The roots of data engineering can be traced back to the 1970s with the development of relational databases and SQL. By the 1980s, businesses began implementing data warehouses, leading to the formalization of the term by Bill Inmon in 1989. Ralph Kimball also contributed significantly through his eponymous modeling techniques.

:p What were some key developments in data warehousing during the 1970s-2000?
??x
Key developments included the creation of relational databases and SQL, which enabled structured storage and querying of business data. Data warehouses were formalized with Inmon's term and Kimball's modeling techniques, providing a foundation for scalable analytics.

```java
// Example of creating a simple SQL query to retrieve data from a database
public class QueryExample {
    public void executeQuery(String query) {
        // Code to connect to the database and run the query
    }
}
```
x??
...
#### Data Warehousing Roles (1980-2000)
Background context: The need for specialized roles emerged as businesses required tools and pipelines for reporting and BI. Roles like BI engineer, ETL developer, and data warehouse engineer were created to address these needs.

:p What roles emerged during the early days of data warehousing?
??x
Roles such as BI engineer, ETL developer, and data warehouse engineer arose to meet the demands of implementing and managing data warehouses effectively.

```java
// Example pseudocode for an ETL process
public class EtlProcess {
    public void transformData() {
        // Code to transform raw data into a format suitable for the data warehouse
    }
}
```
x??
...
#### Web Applications Emergence (Mid-1990s)
Background context: The mid-1990s saw the rise of web applications, leading to new backend systems like servers, databases, and storage. These systems were often expensive, monolithic, and heavily licensed.

:p What changes did web applications bring to the tech industry?
??x
Web applications brought about a shift towards more scalable and flexible systems. They required cost-effective, scalable, available, and reliable infrastructure due to the vast amounts of data they generated.

```java
// Example pseudocode for managing server scalability in a cloud environment
public class ServerManager {
    public void scaleServers(int newCapacity) {
        // Code to add or remove servers based on demand
    }
}
```
x??
...
#### Data Engineering Evolution (Early 2000s)
Background context: In the early 2000s, companies like Yahoo and Google faced challenges with traditional monolithic databases as data growth outpaced their capabilities. They needed new, distributed systems to handle this scale.

:p What issues did companies face in the early 2000s?
??x
Companies encountered limitations with traditional databases due to data volume. They required more cost-effective, scalable, available, and reliable systems to manage growing datasets.

```java
// Example pseudocode for a distributed storage system
public class DistributedStorage {
    public void storeData(String key, String value) {
        // Code to distribute data across multiple nodes in the cluster
    }
}
```
x??
...
#### Commodity Hardware and Innovation (Early 2000s)
Background context: As commodity hardware became cheaper and more ubiquitous, innovations like distributed computation and storage on massive computing clusters allowed for new approaches.

:p What role did commodity hardware play in data engineering?
??x
Commodity hardware's affordability enabled the development of scalable and cost-effective systems. Innovations facilitated distributed computing, breaking apart monolithic services to handle large-scale data processing.

```java
// Example pseudocode for a simple map-reduce operation
public class MapReduce {
    public void mapData(String key, String value) {
        // Code to process data in parallel across multiple nodes
    }
}
```
x??
...
#### Big Data Definition
Background context explaining the concept. The Oxford English Dictionary defines big data as "extremely large data sets that may be analyzed computationally to reveal patterns, trends, and associations, especially relating to human behavior and interactions." Another famous description is the three Vs of data: velocity, variety, and volume.
:p What does the Oxford English Dictionary define big data as?
??x
The definition provided by the Oxford English Dictionary states that big data refers to "extremely large data sets that may be analyzed computationally to reveal patterns, trends, and associations, especially relating to human behavior and interactions." This definition highlights the essence of big data in terms of its analytical potential.
x??

---
#### Google File System and MapReduce
Background context explaining the concept. In 2003, Google published a paper on the Google File System, and shortly after that, in 2004, a paper on MapReduce, an ultra-scalable data-processing paradigm. These publications were significant as they laid the groundwork for modern big data technologies.
:p What did Google publish in 2003 and 2004?
??x
Google published two influential papers: one on the Google File System (GFS) in 2003, which focused on scalable storage systems, and another on MapReduce in 2004, which introduced a scalable data-processing paradigm. These publications were pivotal in shaping modern big data technologies.
x??

---
#### Apache Hadoop
Background context explaining the concept. The Google papers inspired engineers at Yahoo to develop and later open source Apache Hadoop in 2006. This new ecosystem attracted software engineers interested in large-scale data problems, contributing significantly to the growth of big data engineering.
:p What did Yahoo engineer(s) do after reading the Google papers?
??x
After reading the Google papers, engineers at Yahoo developed and later open sourced Apache Hadoop in 2006. This open-source ecosystem attracted software engineers who were interested in large-scale data processing, leading to significant advancements in big data engineering.
x??

---
#### Amazon Web Services (AWS)
Background context explaining the concept. Amazon responded to its own data needs by creating various services such as elastic computing environments and scalable storage systems. It then turned these into AWS, becoming the first popular public cloud service provider.
:p What did Amazon create to meet its growing data needs?
??x
Amazon created several core data building blocks including elastic computing environments (Amazon Elastic Compute Cloud, or EC2), infinitely scalable storage systems (Amazon Simple Storage Service, or S3), and highly scalable NoSQL databases (Amazon DynamoDB) to meet its growing data needs. It later turned these services into AWS, which became the first popular public cloud service.
x??

---
#### Public Cloud Revolution
Background context explaining the concept. The rise of AWS marked a significant innovation in the 21st century, transforming how software and data applications are developed and deployed. Other companies like Google, Microsoft, and DigitalOcean followed Amazon's lead to create their own public clouds.
:p What is one of the most significant innovations of the 21st century according to this text?
??x
One of the most significant innovations of the 21st century, as mentioned in the text, was the rise of the public cloud. This innovation transformed how software and data applications are developed and deployed.
x??

---
#### Batch vs Event Streaming
Background context explaining the concept. The transition from batch computing to event streaming marked a new era of "real-time" big data. Batch processing deals with large datasets in batches, while event streaming processes continuous streams of data events.
:p What is the difference between batch computing and event streaming?
??x
Batch computing processes large datasets in batches, typically for historical or retrospective analysis. Event streaming, on the other hand, processes continuous streams of data events in real-time, enabling near-instantaneous responses to new data inputs.
x??

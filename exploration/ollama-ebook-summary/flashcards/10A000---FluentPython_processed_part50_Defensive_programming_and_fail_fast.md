# Flashcards: 10A000---FluentPython_processed (Part 50)
...
Background context explaining the need for classification systems, including phenetics and cladistics. Discuss how DNA analysis has influenced biological taxonomy.
:p What is cladistics?
??x
Cladistics focuses on characteristics that are inherited from common ancestors rather than those independently evolved, making it a more accurate method in many cases due to advancements like cheap and rapid DNA sequencing.
x??

---

#### Waterfowl Taxonomy Example
Provide examples of how waterfowl have been reclassified based on DNA analysis. Explain the impact of these changes on different contexts such as cooking versus disease susceptibility.
:p How does cladistics affect the classification of sheldgeese, shelducks, white-winged wood ducks, and Muscovy ducks?
??x
Cladistics has led to the reclassification of waterfowl based on shared ancestry. For instance, the white-winged wood duck was moved from its previous genus into a new one due to DNA analysis showing it is more closely related to shelducks than previously thought.
x??

---

#### Duck Typing vs Goose Typing
Explain the difference between duck typing and abstract base classes (ABCs) in Python. Discuss how ABCs can break strong coupling typically associated with inheritance.
:p What is goose typing, and why might it be useful?
??x
Goose typing refers to using `isinstance(obj, cls)` where `cls` is an abstract base class defined by `abc.ABCMeta`. This approach allows classes to be considered subclasses of ABCs without inheriting from them directly, breaking strong coupling. It can be particularly useful in scenarios requiring flexibility and loose type checking.
x??

---

#### Abstract Base Classes (ABCs) and register Method
Describe the use and benefits of abstract base classes in Python, including the `register` method for declaring virtual subclasses.
:p How does the `register` method work with ABCs?
??x
The `register` method allows end-user code to declare that a certain class becomes a "virtual" subclass of an ABC. This is done by ensuring the registered class meets the ABC’s method name and signature requirements, as well as its semantic contract. However, the class need not inherit from the ABC explicitly.
x??

---

#### Practical Advantages of ABCs
Discuss the practical benefits of using abstract base classes in Python programming, such as breaking rigid inheritance hierarchies.
:p Why might one prefer to use ABCs over concrete classes?
??x
Using ABCs can break the rigidity and strong coupling associated with traditional inheritance. This makes code more flexible and easier to extend without enforcing a strict class hierarchy. For example, `isinstance(obj, abc.ABC)` checks if an object meets the contract defined by an abstract base class.
x??

---

#### Inheritance and Duck Typing
Compare and contrast inheritance and duck typing in object-oriented programming, highlighting their respective strengths and weaknesses.
:p What is the advantage of using ABCs over concrete classes?
??x
ABCs offer a way to define contracts that must be met without enforcing an explicit inheritance relationship. This approach can reduce rigid class hierarchies and promote more modular and flexible code design. It aligns with the principle "all non-leaf classes should be abstract" (Scott Meyers, Item 33 in his book).
x??

---

#### Example of ABC Usage
Provide a simple example of defining an ABC and using `register` to declare a virtual subclass.
:p How would you define an ABC for a numeric type and register a new class as its virtual subclass?
??x
You can define an ABC for numeric types and use the `register` method to declare a new class as its virtual subclass.
...
# Registering a new class as a virtual subclass of Numeric
Numeric.register(int)  # int is now recognized as a subclass of Numeric without inheriting from it directly.
```
x??

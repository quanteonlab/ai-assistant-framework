...
Relevant formulas:
\[ N(t) \sim \text{Binomial}\left(\frac{t}{\delta}, \lambda \delta + o(\delta)\right) \rightarrow \text{Poisson}\left(\lambda t + o(\delta)/\delta\right) \]

:p What is the approximation for \(N(t)\) as δ approaches zero?
??x
...
\[ M(t) = \text{Poisson}(\lambda_2 t) \]

:p What is the merged Poisson process rate when combining two independent Poisson processes?
??x
...
Relevant formulas:
\[ P\{N_A(t) = n, N_B(t) = m\} = e^{-\lambda t} \left(\frac{\lambda tp}{n!}\right)^n \cdot \left(e^{-\lambda t(1-p)} \left(\frac{\lambda t (1-p)}{m!}\right)^m\right) \]

:p How does the Poisson splitting theorem describe the classification of events in a process?
??x
...
:p How can the Poisson splitting theorem be intuitively understood using geometric distribution?
??x
...
\[ P\{N_A(t)=n, N_B(t)=m\} = P\{N_A(t)=n\} \cdot P\{N_B(t)=m\}. \]

:p What is the joint probability of two independent Poisson processes?
??x
...
:p How can we prove the memorylessness of the Exponential distribution?
??x
...
By solving, we get:
\[ P(X > 10 + t | X > 10) = e^{-\lambda t}, \]
which is the same as \( P(X > t) \).
...
- Jobs arriving according to a Poisson process: \( X_j \sim Exp(\lambda) \)
- Service times: \( X_k, X_l \sim Exp(\mu) \)
- The time until the next event (job arrival or service completion) is given by \( \tau_i = \min(X_j, X_k, X_l) \).
x??

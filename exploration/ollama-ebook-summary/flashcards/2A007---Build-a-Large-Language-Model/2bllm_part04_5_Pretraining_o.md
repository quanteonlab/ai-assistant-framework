...
Background context explaining how logits are converted to probabilities using the softmax function. The formula for softmax is: $$\text{softmax}(z_i) = \frac{\exp(z_i)}{\sum_{j} \exp(z_j)}$$:p What does the softmax function do, and what is its formula?
??x

# Flashcards: 10A007---Effective-TypeScript---Dan-Vanderkam_processed (Part 2)
...
Background context: In TypeScript, classes introduce both a type and a value. Interfaces only introduce types. This distinction is crucial when working with polymorphism and type-checking.

:p How do class and interface differ in their impact on TypeScript's type system?
??x
In TypeScript, a class introduces both a type (which can be used for instanceof checks) and a value (the constructor function). An interface only defines the shape of an object without providing any actual implementation. This means that when you use `shape: Shape` in a function, it refers to the type, but when you check `instanceof Rectangle`, you are referring to the constructor function's prototype.
x??

---

#### Type Errors and Code Output

Background context: TypeScript type errors do not prevent code from running. The compiler will generate JavaScript code even if there are type issues. This is often surprising for developers coming from statically typed languages like C or Java.

:p Why can code with type errors still produce output in TypeScript?
??x
In TypeScript, the presence of compile-time errors does not stop the generation of JavaScript code. The TypeScript compiler generates a JavaScript file regardless of any type-checking issues, as long as the generated JavaScript is valid. This means that even if your TypeScript code has errors, it can still run and produce output, but you should investigate these errors to ensure correctness.
x??
...
Background context: When working with TypeScript, it's important to distinguish between compiling (code generation) and type checking. The compiler will generate JavaScript as long as the generated code is valid, even if there are type errors.

:p Why might someone incorrectly say that their TypeScript "doesn't compile"?
??x
People may mistakenly say that their TypeScript "doesn't compile" when they actually mean that it has errors in its type-checking. However, this statement is not technically accurate because the TypeScript compiler will still generate JavaScript as long as the output code is valid, regardless of any type errors. The correct term should be that the code does not type check.
x??
...
Background context: In TypeScript, performing type operations such as narrowing a variable's type (e.g., using `if` statements) does not affect the runtime value. It only changes how the type checker perceives the value.

:p How do type operations in TypeScript differ from their effect on runtime values?
??x
Type operations in TypeScript can narrow down the type of variables within certain scopes, but these narrowed types do not change the underlying runtime value. The type information is used by the compiler for error checking and type inference but does not alter how the actual JavaScript code operates at runtime.

For example:
```typescript
let x: string | number;
if (typeof x === 'number') {
    // At this point, TypeScript treats `x` as a number
} else {
    // But here, `x` is still of type `string | number`
}
```

This distinction helps in writing more robust and type-safe code.
x??

...
$$-\frac{1}{2m}\frac{d^2\Psi(r)}{dr^2} + \int dr' V(r, r') \Psi(r') = E \Psi(r).$$:p What is the transformation of the particle's interaction with a many-body medium to an effective one-particle potential?
??x
...
\end{cases}$$where $\alpha$ is a parameter related to the potential.
...
$$R(k_0, k_0) = - \tan(\delta_l)$$where $\rho = 2m k_0$.
...
$$\mathop{\text{P.V.}}\int_{-\infty}^{+\infty}\frac{f(k)}{k-k_0} dk = \lim_{\epsilon \to 0^+} \left( \int_{k_0 - \epsilon}^{-\infty} f(k) \frac{dk}{k-k_0} + \int_{+\infty}^{k_0 + \epsilon} f(k) \frac{dk}{k-k_0} \right)$$:p What is the formula for the Cauchy principal-value prescription?
??x
...
$$\mathop{\text{P.V.}}\int_{-\infty}^{+\infty}\frac{f(k)}{k-k_0} dk = \int_{-\infty}^{+\infty}[f(k) - f(k_0)] \frac{dk}{(k-k_0)^2}$$:p How does the principal-value integral relate to a simpler subtraction?
??x
...
$$R_i - D V R = [1 - D V] R = V$$where $ D $ combines denominators and weights, and $ R $ and $ V $ are vectors containing unknowns and values respectively. The indices $ i$ range from 0 to N.
...
$$V(k', k) = -\frac{|\lambda|^2}{mk'k} \sin(k'b) \sin(kb)$$where $ m $is the mass,$\lambda $ is a parameter related to the strength of the potential, and$b$ is the range of the delta function.

:p What is the analytic solution for the phase shift $\delta_0$?
??x
The analytic solution for the phase shift $\delta_0$ is:
$$\tan\delta_0 = \frac{\lambda b \sin^2(kb)}{kb - \lambda b \sin(kb) \cos(kb)}$$:p What are the parameters used in the problem, and how do they compare to the analytic solution?
??x
...
$$R(k_0, k_0) = -\tan \delta \rho$$where $\rho = 2m k_0$.
...
$$\tan\delta_0 = \frac{\lambda b \sin^2(kb)}{kb - \lambda b \sin(kb) \cos(kb)}$$:p What is the recommended method for solving $[F][R] = [V]$?
??x

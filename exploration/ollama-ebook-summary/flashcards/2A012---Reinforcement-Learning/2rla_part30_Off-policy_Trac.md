# Flashcards: 2A012---Reinforcement-Learning_processed (Part 30)
...
$$G_{s,t} \approx V_\pi(S_t, w_t) + \beta_t \sum_{k=t}^\infty \gamma^k (V_\pi(S_k, w_k) - V_\pi(S_{k-1}, w_{k-1}))$$:p How is the truncated version of the general o↵-policy return defined?
??x
...
$$\sum_{t=1}^\infty (w_{t+1} - w_t) = \alpha \sum_{t=1}^\infty \sum_{k=t}^\infty (\gamma^k V_\pi(S_k, w_k) - \gamma^{k-1} V_\pi(S_{k-1}, w_{k-1}))$$:p How is the backward view update derived?
??x
The backward view update is derived by summing the forward view updates over time. This leads to an expression that can be interpreted as an eligibility trace:
...
Where:
- $\beta_{t+1} = \frac{\pi(A_{t+1}|S_{t+1})}{b(A_{t+1}|S_{t+1})}$-$ V_\pi(S_{t+1}, w_t)$ is the action-value function.

The truncated version of this return can be approximated as:
$$G_a^t \approx Q(S_t, A_t, w_t) + 1 \sum_{k=t}^\infty \beta_k (Q(S_k, A_k, w_k) - Q(S_{k-1}, A_{k-1}, w_{k-1}))$$:p How is the action-based o↵-policy return defined?
??x
...
$$\sum_{t=1}^\infty (w_{t+1} - w_t) = \alpha \sum_{t=1}^\infty \sum_{k=t}^\infty (\beta_k Q(S_k, A_k, w_k) - \beta_{k-1} Q(S_{k-1}, A_{k-1}, w_{k-1}))$$:p How is the action-based backward view update derived?
??x
The backward view update for action-based returns is derived by summing the forward view updates over time. This leads to an expression that can be interpreted as an eligibility trace:
...
$$z_t = \alpha_t t \pi(A_t|S_t) z_{t-1} + r_q(S_t, A_t, w_t)$$where $ z_t $is the eligibility trace,$\alpha_t $ is the learning rate, and$\pi(A_t|S_t)$ is the target policy probability for action $ A_t $ in state $S_t$.
x??
...
#### Tree Backup Algorithm Overview
...
$$w_{t+1} = w_t + \alpha (z_t - \beta z_t^b) v_t^T x^{t+1}_b$$where $ z_t $ and $ v_t $ are eligibility traces, and $ x_b$ represents the feature vector under the behavior policy.
x??
...
$$v_{t+1} = v_t + \lambda (\hat{r}_{t+1} + \gamma v_t^b x_t^T - v_t x_t^T) (x_t - x^{t+1}_b)^T$$where $ z_t $,$ v_t $, and$ z_t^b $ are eligibility traces, and $ x_b$ represents the feature vector under the behavior policy.
x??

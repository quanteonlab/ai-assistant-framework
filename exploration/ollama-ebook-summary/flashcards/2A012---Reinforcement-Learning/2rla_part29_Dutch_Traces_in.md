# Flashcards: 2A012---Reinforcement-Learning_processed (Part 29)
...
$$z_i^t = \rho 1_{x_i^t=1} - z_i^{t-1}$$where $ x_i^t$ is the i-th component of the feature vector at time t, and ρ is a decay factor.
...
#### Accumulating Trace vs. Dutch Trace
...
$$z_i^t = \sum_{k=0}^{t-1} F_t F_{t-1} \cdots F_{k+1} x_k$$where $ F_t = I - \alpha x_t x_t^T$ is a forgetting matrix, and α is the step-size parameter.
